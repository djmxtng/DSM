...
public function comprasultimos_10dias(){
	$sql=" SELECT CONCAT(DAY(fecha_hora),'-',MONTH(fecha_hora)) AS fecha, SUM(total_compra) AS total FROM ingreso WHERE estado='Aceptado' GROUP BY fecha_hora ORDER BY fecha_hora ASC LIMIT 0,10 ";
	return ejecutarConsulta($sql);
}

public function ventasultimos_12meses(){
	$sql=" SELECT DATE_FORMAT(fecha_hora,'%M') AS fecha, SUM(total_venta) AS total FROM venta WHERE estado='Aceptado' GROUP BY MONTH(fecha_hora) ORDER BY fecha_hora ASC LIMIT 0,12";
	return ejecutarConsulta($sql);
}
